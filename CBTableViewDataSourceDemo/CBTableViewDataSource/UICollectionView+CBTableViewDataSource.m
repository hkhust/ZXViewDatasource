//
//  UICollectionView+CBTableViewDataSource.m
...
@implementation UICollectionView (CBTableViewDataSource)
...
- (void)cb_makeDataSource:(void(^)(CBTableViewDataSourceMaker * make))maker {
    CBTableViewDataSourceMaker * make = [[CBTableViewDataSourceMaker alloc] initWithCollectionView:self];
    maker(make);
...
- (void)cb_makeSectionWithData:(NSArray *)data {
    ...
    make.data(data);
    make.cell([UICollectionViewCell class]);
    [self registerClass:make.section.cell forCellWithReuseIdentifier:make.section.identifier];
...
//        }
//    }
    id<CBBaseTableViewDataSourceProtocol> ds = (id<CBBaseTableViewDataSourceProtocol>) [CBBaseTableViewDataSource  new];
